...
func (rcv IssueSlice) SortBy(less func(*Issue, *Issue) bool) IssueSlice {
	result := make(IssueSlice, len(rcv))
	copy(result, rcv)
	...
	maxDepth *= 2
	quickSortIssueSlice(result, less, 0, n, maxDepth)
	return result
}
...
func (rcv IssueSlice) GroupByString(fn func(*Issue) string) map[string]IssueSlice {
	result := make(map[string]IssueSlice)
	for _, v := range rcv {
		key := fn(v)
...
func swapIssueSlice(rcv IssueSlice, a, b int) {
	rcv[a], rcv[b] = rcv[b], rcv[a]
}

// Insertion sort
func insertionSortIssueSlice(rcv IssueSlice, less func(*Issue, *Issue) bool, a, b int) {
	for i := a + 1; i < b; i++ {
		for j := i; j > a && less(rcv[j], rcv[j-1]); j-- {
			swapIssueSlice(rcv, j, j-1)
		}
	}
}
...
func siftDownIssueSlice(rcv IssueSlice, less func(*Issue, *Issue) bool, lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
		...
		swapIssueSlice(rcv, first+root, first+child)
		root = child
	}
}

func heapSortIssueSlice(rcv IssueSlice, less func(*Issue, *Issue) bool, a, b int) {
	first := a
	lo := 0
	hi := b - a
	...
	for i := (hi - 1) / 2; i >= 0; i-- {
		siftDownIssueSlice(rcv, less, i, hi, first)
	}

	// Pop elements, largest first, into end of rcv.
	for i := hi - 1; i >= 0; i-- {
		swapIssueSlice(rcv, first, first+i)
		siftDownIssueSlice(rcv, less, lo, i, first)
	}
}
...
func medianOfThreeIssueSlice(rcv IssueSlice, less func(*Issue, *Issue) bool, a, b, c int) {
	m0 := b
	m1 := a
	m2 := c
	// bubble sort on 3 elements
	if less(rcv[m1], rcv[m0]) {
		swapIssueSlice(rcv, m1, m0)
	}
	if less(rcv[m2], rcv[m1]) {
		swapIssueSlice(rcv, m2, m1)
	}
	if less(rcv[m1], rcv[m0]) {
		swapIssueSlice(rcv, m1, m0)
	}
...
func swapRangeIssueSlice(rcv IssueSlice, a, b, n int) {
	for i := 0; i < n; i++ {
		swapIssueSlice(rcv, a+i, b+i)
	}
}

func doPivotIssueSlice(rcv IssueSlice, less func(*Issue, *Issue) bool, lo, hi int) (midlo, midhi int) {
	m := lo + (hi-lo)/2 // Written like this to avoid integer overflow.
	if hi-lo > 40 {
		// Tukey's Ninther, median of three medians of three.
		s := (hi - lo) / 8
		medianOfThreeIssueSlice(rcv, less, lo, lo+s, lo+2*s)
		medianOfThreeIssueSlice(rcv, less, m, m-s, m+s)
		medianOfThreeIssueSlice(rcv, less, hi-1, hi-1-s, hi-1-2*s)
	}
	medianOfThreeIssueSlice(rcv, less, lo, m, hi-1)
	...
	n := min(b-a, a-lo)
	swapRangeIssueSlice(rcv, lo, b-n, n)

	n = min(hi-d, d-c)
	swapRangeIssueSlice(rcv, c, hi-n, n)

	return lo + b - a, hi - (d - c)
}

func quickSortIssueSlice(rcv IssueSlice, less func(*Issue, *Issue) bool, a, b, maxDepth int) {
	for b-a > 7 {
		if maxDepth == 0 {
			heapSortIssueSlice(rcv, less, a, b)
			return
		}
		maxDepth--
		mlo, mhi := doPivotIssueSlice(rcv, less, a, b)
		// Avoiding recursion on the larger subproblem guarantees
		// a stack depth of at most lg(b-a).
		if mlo-a < b-mhi {
			quickSortIssueSlice(rcv, less, a, mlo, maxDepth)
			a = mhi // i.e., quickSortIssueSlice(rcv, mhi, b)
		} else {
			quickSortIssueSlice(rcv, less, mhi, b, maxDepth)
			b = mlo // i.e., quickSortIssueSlice(rcv, a, mlo)
		}
	}
	if b-a > 1 {
		insertionSortIssueSlice(rcv, less, a, b)
	}
}
