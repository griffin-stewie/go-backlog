// Generated by: gen
// TypeWriter: slice
// Directive: +gen on *CustomFieldValue
...
func (rcv CustomFieldValueSlice) SortBy(less func(*CustomFieldValue, *CustomFieldValue) bool) CustomFieldValueSlice {
	result := make(CustomFieldValueSlice, len(rcv))
	copy(result, rcv)
	...
	maxDepth *= 2
	quickSortCustomFieldValueSlice(result, less, 0, n, maxDepth)
	return result
}

// GroupByString groups elements into a map keyed by string. See: http://clipperhouse.github.io/gen/#GroupBy
func (rcv CustomFieldValueSlice) GroupByString(fn func(*CustomFieldValue) string) map[string]CustomFieldValueSlice {
	result := make(map[string]CustomFieldValueSlice)
	for _, v := range rcv {
		key := fn(v)
...
func (rcv CustomFieldValueSlice) GroupByInt(fn func(*CustomFieldValue) int) map[int]CustomFieldValueSlice {
	result := make(map[int]CustomFieldValueSlice)
	for _, v := range rcv {
		key := fn(v)
		result[key] = append(result[key], v)
	}
...
func (rcv CustomFieldValueSlice) GroupByBool(fn func(*CustomFieldValue) bool) map[bool]CustomFieldValueSlice {
	result := make(map[bool]CustomFieldValueSlice)
	for _, v := range rcv {
		key := fn(v)
		result[key] = append(result[key], v)
	}
...
func (rcv CustomFieldValueSlice) First(fn func(*CustomFieldValue) bool) (result *CustomFieldValue, err error) {
	...
	err = errors.New("no CustomFieldValueSlice elements return true for passed func")
	return
}
...
func (rcv CustomFieldValueSlice) Distinct() (result CustomFieldValueSlice) {
	appended := make(map[*CustomFieldValue]bool)
	for _, v := range rcv {
		if !appended[v] {
			result = append(result, v)
			appended[v] = true
...
func (rcv CustomFieldValueSlice) Shuffle() CustomFieldValueSlice {
	numItems := len(rcv)
	result := make(CustomFieldValueSlice, numItems)
	copy(result, rcv)
...
func swapCustomFieldValueSlice(rcv CustomFieldValueSlice, a, b int) {
	rcv[a], rcv[b] = rcv[b], rcv[a]
}

// Insertion sort
func insertionSortCustomFieldValueSlice(rcv CustomFieldValueSlice, less func(*CustomFieldValue, *CustomFieldValue) bool, a, b int) {
	for i := a + 1; i < b; i++ {
		for j := i; j > a && less(rcv[j], rcv[j-1]); j-- {
			swapCustomFieldValueSlice(rcv, j, j-1)
		}
	}
}
...
func siftDownCustomFieldValueSlice(rcv CustomFieldValueSlice, less func(*CustomFieldValue, *CustomFieldValue) bool, lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
		...
		swapCustomFieldValueSlice(rcv, first+root, first+child)
		root = child
	}
}

func heapSortCustomFieldValueSlice(rcv CustomFieldValueSlice, less func(*CustomFieldValue, *CustomFieldValue) bool, a, b int) {
	first := a
	lo := 0
	hi := b - a
	...
	for i := (hi - 1) / 2; i >= 0; i-- {
		siftDownCustomFieldValueSlice(rcv, less, i, hi, first)
	}

	// Pop elements, largest first, into end of rcv.
	for i := hi - 1; i >= 0; i-- {
		swapCustomFieldValueSlice(rcv, first, first+i)
		siftDownCustomFieldValueSlice(rcv, less, lo, i, first)
	}
}
...
func medianOfThreeCustomFieldValueSlice(rcv CustomFieldValueSlice, less func(*CustomFieldValue, *CustomFieldValue) bool, a, b, c int) {
	m0 := b
	m1 := a
	m2 := c
	// bubble sort on 3 elements
	if less(rcv[m1], rcv[m0]) {
		swapCustomFieldValueSlice(rcv, m1, m0)
	}
	if less(rcv[m2], rcv[m1]) {
		swapCustomFieldValueSlice(rcv, m2, m1)
	}
	if less(rcv[m1], rcv[m0]) {
		swapCustomFieldValueSlice(rcv, m1, m0)
	}
...
func swapRangeCustomFieldValueSlice(rcv CustomFieldValueSlice, a, b, n int) {
	for i := 0; i < n; i++ {
		swapCustomFieldValueSlice(rcv, a+i, b+i)
	}
}

func doPivotCustomFieldValueSlice(rcv CustomFieldValueSlice, less func(*CustomFieldValue, *CustomFieldValue) bool, lo, hi int) (midlo, midhi int) {
	m := lo + (hi-lo)/2 // Written like this to avoid integer overflow.
	if hi-lo > 40 {
		// Tukey's Ninther, median of three medians of three.
		s := (hi - lo) / 8
		medianOfThreeCustomFieldValueSlice(rcv, less, lo, lo+s, lo+2*s)
		medianOfThreeCustomFieldValueSlice(rcv, less, m, m-s, m+s)
		medianOfThreeCustomFieldValueSlice(rcv, less, hi-1, hi-1-s, hi-1-2*s)
	}
	medianOfThreeCustomFieldValueSlice(rcv, less, lo, m, hi-1)
	...
	for {
		for b < c {
			if less(rcv[b], rcv[pivot]) { // rcv[b] < pivot
				...
				swapCustomFieldValueSlice(rcv, a, b)
				a++
				b++
			} else {
				break
		...
		for b < c {
			if less(rcv[pivot], rcv[c-1]) { // rcv[c-1] > pivot
				...
				swapCustomFieldValueSlice(rcv, c-1, d-1)
				c--
				d--
			} else {
				break
		...
		swapCustomFieldValueSlice(rcv, b, c-1)
		b++
		c--
	}
	...
	n := min(b-a, a-lo)
	swapRangeCustomFieldValueSlice(rcv, lo, b-n, n)

	n = min(hi-d, d-c)
	swapRangeCustomFieldValueSlice(rcv, c, hi-n, n)

	return lo + b - a, hi - (d - c)
}

func quickSortCustomFieldValueSlice(rcv CustomFieldValueSlice, less func(*CustomFieldValue, *CustomFieldValue) bool, a, b, maxDepth int) {
	for b-a > 7 {
		if maxDepth == 0 {
			heapSortCustomFieldValueSlice(rcv, less, a, b)
			return
		}
		maxDepth--
		mlo, mhi := doPivotCustomFieldValueSlice(rcv, less, a, b)
		// Avoiding recursion on the larger subproblem guarantees
		// a stack depth of at most lg(b-a).
		if mlo-a < b-mhi {
			quickSortCustomFieldValueSlice(rcv, less, a, mlo, maxDepth)
			a = mhi // i.e., quickSortCustomFieldValueSlice(rcv, mhi, b)
		} else {
			quickSortCustomFieldValueSlice(rcv, less, mhi, b, maxDepth)
			b = mlo // i.e., quickSortCustomFieldValueSlice(rcv, a, mlo)
		}
	}
	if b-a > 1 {
		insertionSortCustomFieldValueSlice(rcv, less, a, b)
	}
}
